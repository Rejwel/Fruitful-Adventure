...
public class FloatingObj : MonoBehaviour
{
    ...
    [SerializeField] private float _jumpRange = 20f;
    ...
    private void RotateObject(Transform objectTransform)
    {
        objectTransform.SetPositionAndRotation(
            new Vector3(objectTransform.position.x, objectTransform.position.y + ((float)Math.Sin(Time.time) * _jumpRange) / 1000, objectTransform.position.z), 
